...
var fft = function fft(signal) {
  ...
  var bitReversedIndices = bitReverseArray(N);
  ...
  for (var n = 1; n <= logN; n++) {
    ...
    for (var k = 0; k < currN / 2; k++) {
      ...
      for (var m = 0; m < N / currN; m++) {
        ...
        var odd = multiply(twiddle, currOddIndexSample);

        var subtractionResult = subtract(currEvenIndexSample, odd);
        complexSignal.real[currOddIndex] = subtractionResult.real;
        complexSignal.imag[currOddIndex] = subtractionResult.imag;

        var additionResult = add(odd, currEvenIndexSample);
...
var ifft = function ifft(signal) {
  ...
  for (var i = 0; i < N; i++) {
    ...
    var conjugateSample = conj(currentSample);
